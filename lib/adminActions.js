...
// --- LOGIKA GALERI (Tabel: galeri) ---

/**
 * Handle penambahan galeri baru, termasuk upload gambar.
 * @param {object} galleryData Data galeri (caption)
 * @param {File | null} imageFile File gambar untuk diupload
 */
export async function handleAddGallery(galleryData, imageFile) {
  try {
    let imageUrl = null;

    // 1. Upload Gambar jika ada
    if (imageFile) {
      imageUrl = await uploadFileToStorage(imageFile, "gallery-images"); // Subfolder 'gallery-images' di bucket 'desa-assets'
      if (!imageUrl) {
        return {
          success: false,
          message: "Gagal mengupload gambar ke Supabase Storage.",
    ...
    // 2. Insert data galeri ke tabel galeri
    const dataToInsert = {
    ...
    if (insertError) {
      throw new Error(`Insert galeri failed: ${insertError.message}`);
    }
    ...
    return {
      success: true,
      message:
        "Galeri baru berhasil ditambahkan dan beranda diperbarui secara instan! **PERINGATAN: Pastikan kolom caption dan image_url sudah ada di tabel galeri Supabase.**",
    };
  } catch (error) {
    console.error("handleAddGallery error:", error);
    return {
      success: false,
      message: error.message || "Terjadi kesalahan saat menambahkan galeri.",
    };
  }
}
